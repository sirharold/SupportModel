#!/usr/bin/env python3
"""
Script simple para generar código de Colab para evaluación acumulativa
"""
...
def generate_colab_code(config: Dict = None) -> str:
    """Genera código completo para ejecutar en Google Colab"""
    ...
    code = f'''# 🚀 Evaluación Acumulativa de Embeddings - Google Colab GPU
# Generado el {config['timestamp']}
...
def main():
    """Función principal para generar y guardar código"""
    
    print("🚀 Generador de Código para Google Colab")
    print("=" * 50)
    ...
    # Generar código
    print("📝 Generando código para Colab...")
    colab_code = generate_colab_code(config)
    
    # Guardar archivo
    output_file = f"Cumulative_Evaluation_Colab_{int(time.time())}.py"
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(colab_code)
    
    print(f"✅ Código generado y guardado en: {output_file}")
    print(f"📊 Tamaño del código: {len(colab_code)} caracteres")
...
4. **Descarga resultados**:
   - Se guardarán en Google Drive y localmente
   - Archivos JSON y CSV listos para importar

🎯 ¡Listo para procesamiento acelerado con GPU!
""")
